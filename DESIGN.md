...
- Add swipe-trailing actions on each loot row: **Use** and **Sell** — both behave identically: decrement `quantity` by 1, insert the corresponding event (`.used` / `.sold`), and delete the item if `quantity` reaches 0; the only difference is the event type logged
...
- **Soft-delete and zero-quantity hiding instead of physical deletion**: items are never removed from the SwiftData store. Use/Sell decrements `quantity`; the item disappears from the loot list when `quantity == 0`. The Delete swipe action sets `isDeleted = true`; the item disappears immediately. The loot list `@Query` filters `isDeleted == false && quantity > 0`. **Trade-off:** soft-deleted and zero-quantity items accumulate in the store indefinitely. A future housekeeping pass could purge items that are both `isDeleted` and have no remaining events referencing them.
- **`LootEvent` stores `itemID: UUID`** — the `id` of the source `LootItem` — for undo lookup. No item snapshot is stored. `undoEvent(_:in:)` is a free function that fetches by UUID, then: `.found` → `isDeleted = true`; `.used`/`.sold` → `quantity += 1`; `.deleted` → `isDeleted = false`. The event is removed from the log after undo. **Trade-off:** if the item is physically deleted outside normal flows (e.g. cascade-deleted with its campaign), the undo fetch returns nil and the undo is a no-op (event is still removed).
- **Delete is now a logged event (`.deleted`)** — so it appears in the EventLogView and can be undone from there, consistent with Use and Sell.
